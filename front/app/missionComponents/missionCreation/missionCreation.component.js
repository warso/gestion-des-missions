...
class controller {
  ...
  addMission () {
    ...
    if (!this.mission.utilisateur) {
      alert('alert1')
    } else if (this.moment(this.mission.debut) > this.moment(this.mission.fin)) { this.errorValid = true } else if ((this.mission.transport === 'AVION') & (this.moment(this.mission.debut).format('YYYY/MM/DD') - this.moment(new Date()).format('YYYY/MM/DD') <= 7)) { this.errorValid1 = true } else if (this.moment(this.mission.debut).format('YYYY/MM/DD') === this.moment(new Date()).format('YYYY/MM/DD')) {
